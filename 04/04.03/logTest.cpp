...
static u_char *ngx_sprintf_num(u_char *buf, u_char *last, uint64_t ui64, u_char zero, uintptr_t hexadecimal, uintptr_t width)
{
    ...
    u_char *temp_last = temp + NGX_UINT64_LEN; // temp_last指向temp最后一个元素位置(不使用)
    u_char *p = temp_last;
    // 十进制数1234567对应十六进制数12D687
    if (hexadecimal == 0) // 十进制%d
    {
        do
        {
...
u_char *ngx_vslprintf(u_char *buf, u_char *last, const char *fmt, va_list args)
{
    while (*fmt && buf < last)
    {
        if (*fmt == '%') // 单字符循环, %开头字符串需要被可变参数替换
        {
            //-----------------变量初始化工作开始-----------------
            u_char zero = (u_char)((*++fmt == '0') ? '0' : ' '); //%0, 填充字符'0'或' '
            uintptr_t width = 0;                                 // 对%d，%f有效，%6d, 数值宽度
            uintptr_t sign = 1;                                  //%d，有符号, 符号位
            uintptr_t hex = 0;                                   // 0：10进制，1：16进制小写字母，2：16进制大写字母, 16进制
            uintptr_t frac_width = 0;                            //%.10f, 小数点后数字宽度
            int64_t i64 = 0;                                     //%d解析可变参得到数值, 保存%d对应的可变参
            uint64_t ui64 = 0;                                   //%ud解析可变参得到数值,保存%ud对应的可变参，和%f对应可变参的整数部分
            //-----------------变量初始化工作结束-----------------
            ...
            while (1)
            {
                ...
                case '.': // 其后边必须跟个数字，必须与%f配合使用，形如%.10f
                    fmt++;
                    while (*fmt >= '0' && *fmt <= '9') // 得到%.后小数部分数字，比如%.16
            ...
            case 's': // 显示字符串
            {
                u_char *p = va_arg(args, u_char *); //; // 保存%s对应的可变参
                while (*p && buf < last)
                    *buf++ = *p++;
                fmt++;
            }
            ...
            {
                double f = va_arg(args, double); // 保存%f对应的可变参
                if (f < 0)
                {                 // 负数的处理
                    *buf++ = '-'; // 单独搞个负号出来
                    f = -f;       // 那这里f应该是正数了!
                ...
                uint64_t frac = 0; // 保存%f对应的可变参的小数部分

                // 如果要求小数点后显示多少位小数
                if (frac_width) // 如果是%d.2f，那么frac_width就会是这里的2
                {
                    uintptr_t scale = 1; // 小数部分缩放倍数

                    for (uintptr_t n = 0; n < frac_width; n++) // 小数部分循环计数
                        scale *= 10;                           // 这可能溢出哦
...
static u_char *ngx_log_errno(u_char *buf, u_char *last, int err)
{
    ...
    if (buf + leftlen + len + rightlen <= last)
    {
...
void ngx_log_stderr(int err, const char *fmt, ...)
{
    ...
    u_char *p = ngx_cpymem(errstr, "nginx: ", strlen("nginx: "));
...
// g++ logTest.cp -o logTest & ./logTest
