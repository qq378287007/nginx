...
void ngx_log_stderr(int err, const char *fmt, ...)
{
    ...
    u_char *last = errstr + NGX_MAX_ERROR_STR; // last指向整个buffer最后去了【指向最后一个有效位置的后面也就是非有效位】，作为一个标记，防止输出内容超过这么长,
                                               // 其实我认为这有问题，所以我才在上边errstr[NGX_MAX_ERROR_STR+1]; 给加了1
                                               // 比如你定义 char tmp[2]; 你如果last = tmp+2，那么last实际指向了tmp[2]，而tmp[2]在使用中是无效的

    u_char *p = ngx_cpymem(errstr, "nginx: ", 7); // p指向"nginx: "之后
...
void ngx_log_init()
{
    ...
    u_char *plogname = (u_char *)p_config->GetString("Log");
    if (plogname == NULL)                        // 没读到，就要给个缺省的路径文件名了
    ...
    // size_t nlen = strlen((const char *)plogname);
