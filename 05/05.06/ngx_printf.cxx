...
u_char *ngx_slprintf(u_char *buf, u_char *last, const char *fmt, ...)
{
    ...
    u_char *p = ngx_vslprintf(buf, last, fmt, args);
    va_end(args); // 释放args
    return p;
}

u_char *ngx_snprintf(u_char *buf, size_t max, const char *fmt, ...) // max指明了缓冲区结束位置
{
    va_list args;
    va_start(args, fmt);
    u_char *p = ngx_vslprintf(buf, buf + max, fmt, args);
    va_end(args);
    return p;
}
